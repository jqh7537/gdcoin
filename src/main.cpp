...
// Copyright (c) 2014 gdcoin Developers
...
//uint256 hashGenesisBlock("0x08dc7211a373880f8cea5834e41de6f5ffd5b476cbec70ea43945c727179c7c9");
uint256 hashGenesisBlock("0x29bee4fc00c6fb655c92394a589afbf3b1bda9b4be952f791288d864da238fa4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gdcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gdcoin Signed Message:\n";
...
{
    // Gdcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//static const int64 nTargetTimespan = 3*60*60;//3 * 60 * 60; // Gdcoin:every 3 hours
//static const int64 nTargetSpacing =  3*60; // Gdcoin:3 minutes
//static const int64 nInterval = nTargetTimespan / nTargetSpacing;

static int64 nTargetTimespan = 2*60*60;//2 * 60 * 60; // Gdcoin:every 2 hours
static int64 nTargetSpacing =  60; // Gdcoin:1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gdcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Gdcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "start gdc 20140309";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394463145;//2014-3-9 18:17:59 //1391830000;//2014-2-8 11:26:40
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 7923375;//
        ...
        assert(block.hashMerkleRoot == uint256("0x4985409691ce3b06765dfabaca6dee30e764b4da03120898bdcad7f53036ca14"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gdcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GdcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GdcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GdcoinMiner(CWallet *pwallet)
{
    printf("GdcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GdcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GdcoinMiner, pwallet));
}
